...
extension String{
    ...
    func REContains(str: String) -> Bool{
        let regex = ".*?" + str.map(String.init).joined(separator: ".*?") + ".*?"
        guard let RE = try? NSRegularExpression(pattern: regex, options: .caseInsensitive) else {
            return false
        }
        if let _ = RE.firstMatch(in: self, options: .reportProgress, range: NSRange(location: 0, length: self.count)){
            return true
        }else{
            return false
        }
...
fileprivate class GBig {
    fileprivate static let big5 =
    ...
    private static var _dict: [Character: Character] = [:]
    ...
    fileprivate static func simplify<T>(_ ch: T) -> String where T: StringProtocol{
    ...
    //    fileprivate static func traditionalize<T>(_ ch: T) -> String where T: StringProtocol{
    ////    return String(ch.map { c in
